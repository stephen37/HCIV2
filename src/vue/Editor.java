...
@SuppressWarnings({ "unused", "resource", "static-access", "serial",
		"unchecked" })
public class Editor extends JFrame implements DropTargetListener, Serializable,
		KeyListener {
	...
	public Editor(String theTitle, int width, int height, ToolBar tool) {
		frame = this;
		widthWindow = width;
		heightWindow = height;
		title = theTitle;
...
//		new DropTarget(canvas, this);
	...
	public void initCanvas() {
		...
		contour.setMinimumSize(new Dimension(canvas.getWidth()+2, canvas.getHeight()+2));
		contour.setSize(new Dimension(canvas.getWidth()+2, canvas.getHeight()+2));
		contour.setBackground(Color.black);
		
		
		contour.add(canvas);
		canvasPanel.add(contour);
...
//		sepa.setSize(new Dimension(widthWindow, 2));
